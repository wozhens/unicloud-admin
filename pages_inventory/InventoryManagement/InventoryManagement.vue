	...
	export default {
		data() {
			...
			return {
				...
				table1:{
					// 表格数据请求地址
					action:"InventoryManagement/db_api/sys/getList",
					// 表格字段显示规则
					columns:[
						{ key:"name", title:"名称", type:"text", width:220 },
						{ key:"stock", title:"库存", type:"number", width:200, sortable:"custom" },
						{ key:"remark", title:"备注", type:"text", width:100 },
				...
				form1:{
					...
					props: {
						...
						columns:[
							...
							{ key:"stock", title:"库存", type:"number", placeholder:"请输入库存" },
		...
		methods: {
			...
			addBtn(){
				that.resetForm();
				that.form1.props.action = 'InventoryManagement/db_api/sys/add';
			...
			// 显示修改页面
			updateBtn({ item }){
				that.form1.props.action = 'InventoryManagement/db_api/sys/update';
			...
			deleteBtn({ item, deleteFn }){
				deleteFn({
					action: "InventoryManagement/db_api/sys/delete",
					data:{
						_id: item._id
					},
					refresh: true
